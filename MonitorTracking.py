...
=============================================================================
  FIXES over v3:
  EYE EDGE ACCURACY:
    - Calibration corner points pushed to 0.02/0.98 so the polynomial
      has data at the true screen edges (not just 0.08/0.92)
    - Raw gaze range auto-measured during calibration so the model input
      is always normalised to the full [-1, +1] range, not a compressed
      sub-range — this was the main cause of edge clipping
    - Edge-stretch remapping after polynomial: output gently expanded
      past 0/1 boundary before final clip so corners are reachable
    - "E" key: live edge-stretch factor tunable without recalibration

  HAND PALM-SIDE DETECTION:
    - Reads MediaPipe handedness label ("Left"/"Right") each frame
    - Detects palm-vs-back orientation using wrist→middle-MCP cross product
    - Thumb curl logic now flips correctly for both palm-facing and
      back-facing orientations, and for both left and right hands
    - Added "PALM_PUSH" gesture: open palm facing camera → scroll up
      (replaces the old ✋ which required back-of-hand orientation)
    - Fist detection now stable for both orientations

  OTHER:
    - "T" key: toggle between hand-back and palm-facing modes
      (auto-detect is on by default but you can force a mode)
    - Gesture debug line shows detected orientation + handedness
=============================================================================
